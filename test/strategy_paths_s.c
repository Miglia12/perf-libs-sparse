...
#define STRATEGY_PATHS_S_M 24
#define STRATEGY_PATHS_S_K 32
#define STRATEGY_PATHS_S_N 128
#define STRATEGY_PATHS_S_NNZA (STRATEGY_PATHS_S_M * 4)
#define STRATEGY_PATHS_S_NNZB (STRATEGY_PATHS_S_K * 4)

static int test_large_regular_spmv_optimize_and_update_float() {
  ...
  CHECK_TRUE(row_ptr && col_indx && vals && x && y,
             "allocation failure in large spmv test");
  ...
  CHECK_STATUS(perflibs_spmat_export_csr_s(A, 0, &m_out, &n_out, &row_ptr_out,
  ...
  CHECK_STATUS(perflibs_spmv_exec_s(PERFLIBS_SPARSE_OPERATION_NOTRANS, 1.0f, A,
                                    x, 0.0f, y));
  for (perflibs_int_t i = 0; i < m; ++i) {
    CHECK_DOUBLE_NEAR(y[i], (double)width, 1e-5);
  }

  const perflibs_int_t row_update = updated_row;
  const perflibs_int_t col_update = updated_col;
  const float val_update = 2.0f;
  CHECK_STATUS(
      perflibs_spmat_update_s(A, 1, &row_update, &col_update, &val_update));
  memset(y, 0, sizeof(float) * m);
  CHECK_STATUS(perflibs_spmv_exec_s(PERFLIBS_SPARSE_OPERATION_NOTRANS, 1.0f, A,
                                    x, 0.0f, y));
  for (perflibs_int_t i = 0; i < m; ++i) {
    double expected = i == updated_row ? (double)width + 1.0 : (double)width;
    CHECK_DOUBLE_NEAR(y[i], expected, 1e-5);
  }

  for (perflibs_int_t i = 0; i < m; ++i) {
    y[i] = 3.0f;
  }
  CHECK_STATUS(perflibs_spmv_exec_s(PERFLIBS_SPARSE_OPERATION_NOTRANS, 2.0f, A,
                                    x, 1.0f, y));
  for (perflibs_int_t i = 0; i < m; ++i) {
...
static void build_spmm_inputs(perflibs_int_t *row_ptrA,
                              perflibs_int_t *col_indxA, float *valsA,
                              perflibs_int_t *row_ptrB,
                              perflibs_int_t *col_indxB, float *valsB,
                              float *denseA, float *denseB) {
  const perflibs_int_t m = 24;
  const perflibs_int_t k = 32;
  const perflibs_int_t n = 128;

  memset(denseA, 0, sizeof(float) * m * k);
  memset(denseB, 0, sizeof(float) * k * n);

  row_ptrA[0] = 0;
  for (perflibs_int_t i = 0; i < m; ++i) {
    row_ptrA[i + 1] = row_ptrA[i] + 4;
    perflibs_int_t start = i % (k - 3);
...
static int test_spmm_optimized_symbolic_strategy(
    enum perflibs_sparse_hint_value strategy_hint) {
  perflibs_int_t row_ptrA[STRATEGY_PATHS_S_M + 1];
  perflibs_int_t col_indxA[STRATEGY_PATHS_S_NNZA];
  float valsA[STRATEGY_PATHS_S_NNZA];
  perflibs_int_t row_ptrB[STRATEGY_PATHS_S_K + 1];
  perflibs_int_t col_indxB[STRATEGY_PATHS_S_NNZB];
  float valsB[STRATEGY_PATHS_S_NNZB];
  float denseA[STRATEGY_PATHS_S_M * STRATEGY_PATHS_S_K];
  float denseB[STRATEGY_PATHS_S_K * STRATEGY_PATHS_S_N];
  float expected[STRATEGY_PATHS_S_M * STRATEGY_PATHS_S_N];

  build_spmm_inputs(row_ptrA, col_indxA, valsA, row_ptrB, col_indxB, valsB,
                    denseA, denseB);
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, STRATEGY_PATHS_S_M,
              STRATEGY_PATHS_S_N, STRATEGY_PATHS_S_K, 1.0f, denseA,
              STRATEGY_PATHS_S_K, denseB, STRATEGY_PATHS_S_N, 0.0f, expected,
              STRATEGY_PATHS_S_N);

  perflibs_spmat_t A = NULL;
  perflibs_spmat_t B = NULL;
  perflibs_spmat_t C =
      perflibs_spmat_create_null(STRATEGY_PATHS_S_M, STRATEGY_PATHS_S_N);
  perflibs_int_t m_out = -1, n_out = -1;
  ...
  CHECK_STATUS(perflibs_spmat_create_csr_s(&A, STRATEGY_PATHS_S_M,
                                           STRATEGY_PATHS_S_K, row_ptrA,
                                           col_indxA, valsA, 0));
  CHECK_STATUS(perflibs_spmat_create_csr_s(&B, STRATEGY_PATHS_S_K,
                                           STRATEGY_PATHS_S_N, row_ptrB,
                                           col_indxB, valsB, 0));
  ...
  CHECK_STATUS(perflibs_spmat_export_csr_s(C, 0, &m_out, &n_out, &row_ptrC,
                                           &col_indxC, &valsC));
  CHECK_INT_EQ(m_out, STRATEGY_PATHS_S_M);
  CHECK_INT_EQ(n_out, STRATEGY_PATHS_S_N);
  CHECK_TRUE(row_ptrC[STRATEGY_PATHS_S_M] > 0,
  ...
  CHECK_STATUS(perflibs_spmm_exec_s(PERFLIBS_SPARSE_OPERATION_NOTRANS,
                                    PERFLIBS_SPARSE_OPERATION_NOTRANS, 1.0f, A,
                                    B, 0.0f, C));
  CHECK_STATUS(perflibs_spmat_export_dense_s(C, PERFLIBS_ROW_MAJOR, &m_out,
                                             &n_out, &denseC));
  CHECK_FLOAT_ARRAY(denseC, expected, STRATEGY_PATHS_S_M * STRATEGY_PATHS_S_N,
                    1e-4f);
  free(denseC);
...
int main() {
  if (test_large_regular_spmv_optimize_and_update_float() != EXIT_SUCCESS) {
    return EXIT_FAILURE;
  }
  if (test_spmm_optimized_symbolic_strategy(
