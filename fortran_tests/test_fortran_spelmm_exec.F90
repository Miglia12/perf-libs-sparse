...
program test_fortran_spelmm_exec
    use perflibs_sparse
    use iso_c_binding
    implicit none
    ...
    FTYPE, allocatable :: dense_a_alloc(:), dense_b_alloc(:), dense_c_alloc(:), result_alloc(:)
    ...
    integer :: stat, i
    integer :: flags = 0
    integer(kind=perflibs_i8) :: perflibs_mat_a, perflibs_mat_b, perflibs_mat_c
    logical :: is_complex
    ...
    type(c_ptr) :: dense_a_vals, dense_b_vals, dense_c_vals
    FTYPE, pointer :: dense_a_ptr(:), dense_b_ptr(:), dense_c_ptr(:)
    ...
    ! Create CSR matrices
    call perflibs_spmat_create_csr(perflibs_mat_a, m_orig, n_orig, row_ptr, col_indx, vals, flags, stat)
    call check_stat("perflibs_spmat_create_csr")
    call perflibs_spmat_create_csr(perflibs_mat_b, m_orig, n_orig, row_ptr, col_indx, vals, flags, stat)
    call check_stat("perflibs_spmat_create_csr")
    call perflibs_spmat_create_csr(perflibs_mat_c, m_orig, n_orig, row_ptr, col_indx, vals, flags, stat)
    call check_stat("perflibs_spmat_create_csr")

    ! Calculate the reference result
    call get_reference()

    ! Call SpElMM optimize function
    call perflibs_spelmm_optimize(perflibs_sparse_operation_notrans, perflibs_sparse_operation_notrans, &
                                  perflibs_sparse_scalar_one, perflibs_mat_a, perflibs_mat_b, &
                                  perflibs_sparse_scalar_one, perflibs_mat_c, stat)
    call check_stat("perflibs_spelmm_optimize")

    ! Do element-wise multiplication
    call perflibs_spelmm_exec(perflibs_sparse_operation_notrans, perflibs_sparse_operation_notrans, &
                              alpha, perflibs_mat_a, perflibs_mat_b, beta, perflibs_mat_c, stat)
    call check_stat("perflibs_spelmm_exec")
    ...
    write (*,'(A,I1,A,I3)') "Success - test_fortran_spelmm_exec, kind = ", rk, &
...
contains
    ...
    subroutine get_reference()
        ...
        if (libgfortran_mismatch .eq. 1) then
            ...
            if (typeid .eq. 1) then
                ...
                call perflibs_spmat_export_dense_wrapper_s(perflibs_mat_b, perflibs_row_major, m, n, dense_b_vals, stat)
                call check_stat("perflibs_spmat_export_dense_s")
                ...
                call perflibs_spmat_export_dense_wrapper_d(perflibs_mat_b, perflibs_row_major, m, n, dense_b_vals, stat)
                call check_stat("perflibs_spmat_export_dense_d")
                ...
                call perflibs_spmat_export_dense_wrapper_c(perflibs_mat_b, perflibs_row_major, m, n, dense_b_vals, stat)
                call check_stat("perflibs_spmat_export_dense_c")
                ...
                call perflibs_spmat_export_dense_wrapper_z(perflibs_mat_b, perflibs_row_major, m, n, dense_b_vals, stat)
                call check_stat("perflibs_spmat_export_dense_z")
            ...
            call perflibs_spmat_export_dense(perflibs_mat_b, perflibs_row_major, m, n, dense_b_alloc, stat)
            call check_stat("perflibs_spmat_export_dense")
        ...
        do i = 1, m * n
            reference(i) = (alpha * (dense_a_alloc(i) * dense_b_alloc(i))) &
                                                      + (beta * dense_c_alloc(i))
        end do

        deallocate(dense_a_alloc)
        deallocate(dense_b_alloc)
    ...
    subroutine compare_results()
        ...
        do i = 1, m * n
            tol = op_tol_mod * nnz * abs(reference(i)) * eps
            diff = abs(reference(i) - result_alloc(i))
            if (diff > tol) then
                stat = perflibs_status_execution_failure
                call check_stat("spelmm failed tolerance check")
            end if
        end do
    ...
    subroutine check_stat(err_msg)
        ...
        if (stat .ne. perflibs_status_success) then
            write (*,*) err_msg, ' failed in test_fortran_spelmm_exec'
            stop 1
        end if
    end subroutine check_stat

end program test_fortran_spelmm_exec
