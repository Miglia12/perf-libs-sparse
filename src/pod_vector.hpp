...
// This is a deliberately small internal substitute for the subset of
// std::vector-like behavior needed by the library's POD-heavy kernels. It keeps
// allocation and initialization behavior explicit so hot paths remain easy to
// reason about.
...
/** A vector-like container for value types that will survive not having a
 * constructor called (e.g. float/double/std::complex). Calling .resize() on
 * this container will leave any newly allocated data uninitialized, unlike
 * std::vector which will default-initialize it.
 */
template <typename T, typename R = reallocator> class pod_vector {
public:
...
private:
  ...
  template <typename T2, typename R2> friend class pod_vector;
...
public:
  ...
  pod_vector(const pod_vector<T, R> &other) { *this = other; }

  pod_vector(pod_vector<T, R> &&other) { *this = std::move(other); }
  ...
  ~pod_vector() {
    ...
    assert(m_data || m_capacity == 0);
    if (m_data)
      m_realloc.deallocate(m_data);
  }
  ...
  void push_back(T value) {
    if (m_size == m_capacity) {
      size_type new_capacity =
          m_capacity == 0 ? 1 : m_capacity * growth_factor_;
 ...
 *  type.
 */
template <typename R> class pod_vector<void, R> {
public:
  using size_type = std::size_t;
  using pointer = void *;
...
public:
  ...
  template <typename T> pod_vector(const pod_vector<T, R> &other) {
    *this = other;
  }

  template <typename T> pod_vector(pod_vector<T, R> &&other) {
    *this = std::move(other);
  }

  pod_vector(const pod_vector<void, R> &other) { *this = other; }

  pod_vector(pod_vector<void, R> &&other) { *this = std::move(other); }
  ...
  pod_vector<void, R> &operator=(const pod_vector<void, R> &other) {
    ...
    if (other.m_capacity * other.m_elem_size != m_capacity * m_elem_size) {
      m_data =
          m_realloc.reallocate(m_data, other.m_capacity * other.m_elem_size);
    }
    m_capacity = other.m_capacity;
    m_size = other.m_size;
    m_elem_size = other.m_elem_size;
...
template <typename V> struct is_vec_type : std::false_type {};

template <typename T> struct is_vec_type<std::vector<T>> : std::true_type {};

template <typename T, typename R>
struct is_vec_type<perflibs::sparse::pod_vector<T, R>> : std::true_type {};

/**
 * Utility to help check that a template parameter is a vector type
 * (either std::vector or @ref perflibs::sparse::pod_vector for now)
 */
template <typename V> constexpr bool is_vec_type_v = is_vec_type<V>::value;

template <typename T> struct has_complex_value_type : std::false_type {};

template <typename T>
struct has_complex_value_type<std::vector<std::complex<T>>> : std::true_type {};

template <typename T, typename R>
struct has_complex_value_type<perflibs::sparse::pod_vector<std::complex<T>, R>>
    : std::true_type {};
