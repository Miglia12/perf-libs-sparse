...
template <>
void spmv_scs_opt(const perflibs_scs<float> &scs, const float *x, float *y,
...
#pragma omp parallel for schedule(static) shared(tidy) num_threads(nt)
  for (int me = 0; me < nt; me++) {

    {
      ...
      if (nchunks) {
        // Call appropriate assembly kernel
        auto *kernel =
...
#pragma omp atomic capture
    ...
    if (!do_tidy && scs.ntidyup) {
      ...
      // Zero work vector
      perflibs::sparse::pod_vector<float> work(scs.C);
      memset(&work[0], 0, sizeof(float) * scs.C);
