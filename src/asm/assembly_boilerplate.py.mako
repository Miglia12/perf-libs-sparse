...
<%def name="prologue(func_name)">
	...
	%elif target_os == 'windows':
		...
		${func_name}:
	%else:
		<% raise Exception('Unrecognized operating system target in prologue(): ' + target_os) %>
	%endif
</%def>
...
<%def name="epilogue(func_name)">
	...
	%elif target_os == 'windows':
	%else:
		<% raise Exception('Unrecognized operating system target in epilogue(): ' + target_os) %>
	%endif
</%def>
...
<%def name="assert_fn()">
	...
	%elif target_os == 'windows':
		bl _wassert
	%else:
		<% raise Exception('Unrecognized operating system target in assert_fn(): ' + target_os) %>
	%endif
</%def>
